...
namespace Grape.Tests
{
    public abstract class GrapeTestBase : AbpIntegratedTestBase<GrapeTestModule>
    {
        ...
        protected void UsingDbContext(Action<GrapeDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GrapeDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GrapeDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GrapeDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GrapeDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GrapeDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GrapeDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GrapeDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GrapeDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GrapeDbContext, Task<T>> func)
        {
            T result;
